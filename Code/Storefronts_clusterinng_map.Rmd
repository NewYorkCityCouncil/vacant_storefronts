...
```{r, include=FALSE}
leaflet(ct_vacant_clusters.shp %>% filter(cluster==1), options = leafletOptions(minZoom = 10), height = 500, width =  500) %>%
  ...
  addPolygons(weight = 1,
              ...
              stroke = TRUE,
              fillOpacity = 0.8) %>%
  addPolygons(data = ct_vacant_clusters.shp %>% filter(cluster==2) %>% st_as_sf() %>% st_transform('+proj=longlat +datum=WGS84'), weight = 1 ,
              fillColor = "orange",
              color="white",
              stroke = TRUE,
              fillOpacity = 0.8) %>%
  addPolygons(data = ct_vacant_clusters.shp %>% filter(cluster==0) %>% st_as_sf() %>% st_transform('+proj=longlat +datum=WGS84'), weight = 1 ,
              fillColor = "pink",
              color="white",
              stroke = TRUE,
              fillOpacity = 0.6) %>% addPolygons(data = ct.shp %>% mutate(boro_ct201=as.numeric(as.character(boro_ct201))) %>% anti_join(ct_vacant.shp %>% data.frame(), by=c("boro_ct201")), weight = 1,
            ...
            labels = c("vacant: 14% storefronts: 171", "vacant: 13%  storefronts: 36", "vacant: 0% storefronts: 12"),
            title = "Cluster means",
            opacity = 8)

ct_vacant_clusters %>% filter(perc_vacant==0) %>% ungroup() %>% summarize(mean(perc_vacant), mean(total_storefronts))
