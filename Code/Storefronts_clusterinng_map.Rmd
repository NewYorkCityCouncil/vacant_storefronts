...
ct_vacant_dataset_2 <- leased_not_leased_2022[leased_not_leased_2022$reporting_year == '2020 and 2021',] 

clean_vacant_id <- function(data) {
  data$boro_ct201 <- str_extract(data$bb, "^\\d{1}")
  data$census_tract_2 <- as.numeric(data$census_tract) * 100
  data$census_tract_2 <-str_pad(data$census_tract_2, width = 6, pad = "0")
  data <- transform(data, boro_ct201 = paste0(boro_ct201, census_tract_2))
  return(data)
}

ct_vacant_dataset <- clean_vacant_id(ct_vacant_dataset)
ct_vacant_dataset_2 <- clean_vacant_id(ct_vacant_dataset_2)
```
...
ct_vacant <- aggregate(ct_vacant_dataset)
ct_vacant_2 <- aggregate(ct_vacant_dataset)
```
...
ct_vacant.shp <- spatial_prep(ct_vacant)
ct_vacant.shp_2 <- spatial_prep(ct_vacant_2)
```
