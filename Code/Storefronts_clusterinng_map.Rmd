...
table(ct_vacant_clusters_2$cluster)
...
shp_join <- function(ct_vacant_clust, ct_vacant_clust_shp) { joined <- ct_vacant_clust %>% 
  left_join(ct_vacant_clust_shp, by = c("boro_ct201", "cluster_recoded")) %>%
  ungroup() %>% 
  st_as_sf() %>% 
  st_transform('+proj=longlat +datum=WGS84')
return(joined)
}
ct_vacant_clusters.shp <- shp_join(ct_vacant_clusters, ct_vacant_clusters.shp)
```
