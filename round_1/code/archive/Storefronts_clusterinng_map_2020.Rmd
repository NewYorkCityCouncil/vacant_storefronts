...
```{r}
ct_vacant_dataset <- ct_vacant_dataset[ct_vacant_dataset$reporting_year == '2020 and 2021',] 
...
ct_vacant_clusters.shp_2 <- ct_vacant_clusters %>% left_join(ct.shp %>% mutate(boro_ct201=as.character(boro_ct201)), by = c("boro_ct201")) %>% ungroup() %>% st_as_sf() %>% st_transform('+proj=longlat +datum=WGS84')
```


```{r, include=FALSE}
cluster_map <- leaflet(ct_vacant_clusters.shp_2 %>% filter(cluster_recoded==1), options = leafletOptions(minZoom = 10), height = 700, width =  700) %>%
