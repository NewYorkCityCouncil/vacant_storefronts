...
```{r}
# datasets
council_geographies <- read_csv('data/council_geographies.csv')
district_43 <- read_csv('data/store_fronts-district_43.csv')
```
...
popup_mapping <- list(
  ...
  property_street_address_or = "Address",
  primary_business_activity = "Primary Business Activity",
  vacant_on_12_31 = "Occupancy Status"
)
...
for (i in 1:nrow(district_43)) {
  circle_color <- set_circle_color(district_43$vacant_on_12_31[i])
  popup_content <- paste(
